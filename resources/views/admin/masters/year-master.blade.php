                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @can('yearmasters.edit')
                                                <button class="edit-element btn btn-secondary px-2 py-1" title="Edit Yearmaster" data-id="{{ $yearmasters->id }}"><i data-feather="edit"></i></button>
                                            @endcan
                                            @can('yearmasters.delete')
                                                <button class="btn btn-danger rem-element px-2 py-1" title="Delete Yearmaster" data-id="{{ $yearmasters->id }}"><i data-feather="trash-2"></i> </button>
                                            @endcan
                                        </td>
                                    </tr>
                                @endforeach
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (!data.error2)
                    ...
                    .then((action) => {
                        window.location.href = '{{ route('year-master.index') }}';
                    });
                else
...
<script>
    $("#buttons-datatables").on("click", ".edit-element", function(e) {
        ...
        var url = "{{ route('year-master.edit', ':model_id') }}";
        ...
        $.ajax({
            ...
            success: function(data, textStatus, jqXHR) {
                editFormBehaviour();
                if (!data.error) {
                    $("#editForm input[name='edit_model_id']").val(data.yearmasters.id);
                    $("#editForm input[name='title']").val(data.yearmasters.title);
                    $("#editForm input[name='start_date']").val(data.yearmasters.start_date);
                    $("#editForm input[name='end_date']").val(data.yearmasters.end_date);
                    $("#editForm input[name='status']").val(data.yearmasters.status);
                    $("#editForm input[name='freeze_status']").val(data.yearmasters.freeze_status);
                } else {
                    alert(data.error);
                }
            },
...
<script>
    $(document).ready(function() {
        $("#editForm").submit(function(e) {
            ...
            var url = "{{ route('year-master.update', ':model_id') }}";
    ...
    $("#buttons-datatables").on("click", ".rem-element", function(e) {
        ...
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    var model_id = $(this).attr("data-id");
                    var url = "{{ route('year-master.destroy', ':model_id') }}";
